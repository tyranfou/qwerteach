class DashboardsController < ApplicationController
  ...
  def index
    ...
    lessons_without_review = Lesson.joins('LEFT OUTER JOIN reviews ON reviews.subject_id = lessons.teacher_id
      AND reviews.sender_id = lessons.student_id').where(:student => @user.id).where('reviews.id is NULL')
    unpaid_lessons = Lesson.joins(:payments).where(:student => @user.id).where('payments.status = 0')

    @to_do_list = (lessons_without_review + unpaid_lessons).sort_by &:created_at
  end
end
